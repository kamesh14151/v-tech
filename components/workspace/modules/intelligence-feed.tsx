...
export function IntelligenceFeedView() {
  ...
  const [query, setQuery] = useState("enterprise technology AI media");
      ...
      <div className="flex gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="Search topic, entity, keyword…"
            className="w-full pl-8 pr-3 py-2 text-xs font-mono rounded-xl border border-foreground/10 bg-background/50 focus:outline-none focus:border-foreground/30 transition-colors"
          />
        </div>
        <select
          value={sort}
          onChange={(e) => setSort(e.target.value as SortType)}
          className="px-3 py-2 text-xs font-mono rounded-xl border border-foreground/10 bg-background text-foreground focus:outline-none"
        >
