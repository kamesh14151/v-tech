...
function toSignal(a: Article): RiskSignal {
  ...
  const conf = Math.min(99, Math.max(75, a.relevanceScore));
  ...
  return {
    ...
    mentions: Math.min(100, Math.max(15, Math.round(Math.abs(sentScore) * 100))),
    aiExplanation:
      severity === "critical"
...
export function RiskRadarView({ query: propQuery = "" }: { query?: string }) {
  ...
  const activeQuery = propQuery.trim() || "enterprise technology risk crisis";

  const fetchRisks = useCallback(async () => {
    setLoading(true);
    setError(null);
    try {
      const res = await fetch(`/api/news?q=${encodeURIComponent(activeQuery + " negative risk")}&pageSize=20`);
